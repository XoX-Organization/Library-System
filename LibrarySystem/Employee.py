...
class Employee:
    ...
    def database(self):
        try: self._database
        except AttributeError: self._database = DB_Employee.Retrieve()
    ...
    def Login(self, password):
        ...
        if not self.valid_ID:
            logger.info('Invalid %s, please register', self.ID)
        ...
        if 'BCrypt-Pass' in self.database[self.ID].keys():
            ...
            if bcrypt.checkpw(password.encode('utf-8'), hashed_pass.encode('utf-8')):
                logger.info('Login successful as user %s', self.ID)
                logger = remove_handler(logger)
        ...
        logger.info('Password incorrect as user %s', self.ID)
